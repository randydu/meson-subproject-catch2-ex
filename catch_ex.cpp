...
namespace catch_ex {
    namespace {
        ...
        public:
            ...
            void register_test_callback(test_callback_t test_callback) {
                if (cb_test) throw std::logic_error("global test callback can only be registered once!");
        ...
        CallbackMan& man() {
            if (CallbackMan::man_ == nullptr) CallbackMan::man_ = new CallbackMan();
        ...
        public:
            ...
            void testCaseStarting(Catch::TestCaseInfo const& _testInfo) override {
                ...
                if (man().verbose_printing_enabled()) {
                    ...
                    for (auto const& tag : _testInfo.tags) std::cout << tag.c_str() << ", ";
            ...
            void testCaseEnded(Catch::TestCaseStats const& _testCaseStats) override {
                if (man().verbose_printing_enabled()) {
                    ...
                    for (auto const& tag : _testCaseStats.testInfo.tags) std::cout << tag.c_str() << ", ";
    ...
    void register_test_callback(test_callback_t test_callback) {
        if (!test_callback) return;
        man().register_test_callback(test_callback);
    }

    void register_tag_callback(const std::string& tag_or_tags, tag_callback_t tag_callback, bool shared) {
        if (!tag_callback) return;
